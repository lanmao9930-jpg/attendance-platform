  ...
  if (route(pathname, "/api/schedules")) {
    ...
    if (req.method === "POST") {
      ...
      if (mode === "replace-centers") {
        ...
        const previousBatchId = currentBatch.id || legacyScheduleBatchId(activeSchedules);
        const previousBatchName = currentBatch.id
          ? currentBatch.name
          : cleanBatchName(body.previousBatchName || "上一版固定排班", currentBatch.createdAt || now);
        const previousCreatedAt = currentBatch.createdAt || scheduleCreatedAt(activeSchedules) || now;
        if (activeSchedules.length) {
          await saveScheduleHistoryBatch({
            id: previousBatchId,
            ...
            batchCreatedAt: schedule.batchCreatedAt || previousCreatedAt,
            isActive: false,
            archivedAt: now
          })));
        }
        savedSchedules = result.schedules.map((schedule) => ({
          ...schedule,
          batchId: newBatchId,
          batchName,
          batchCreatedAt: now,
      ...
      const batches = await scheduleBatchCatalog(active);
      return json(res, 200, {
        ok: true,
        count: active.length,
        totalStored: batches.reduce((sum, batch) => sum + batch.count, 0),
