...
async function handleApi(req, res) {
  ...
  if (route(pathname, "/api/reviews")) {
    ...
    if (!db.schedules.some((schedule) => schedule.id === scheduleId)) {
...
function normalizeSchedules(source) {
  ...
  source.forEach((item, index) => {
    ...
    const key = [row.value.center, row.value.week, row.value.weekday, row.value.shift, row.value.name].join("|");
...
function normalizeSchedule(item, index) {
  const week = weekNumber(item?.week);
  ...
  if (!weeks.includes(week)) return { ok: false, reason: "周次应为 1 至 20" };
