  ...
  if (route(pathname, "/api/checkins")) {
    ...
    if (req.method === "GET") {
      ...
      const checkins = (db.checkins || []).map((record) => {
        ...
        const isHistorical = !checkinBelongsToBatch(record, activeBatch);
  ...
  if (route(pathname, "/api/schedules")) {
    ...
    if (req.method === "GET") {
      ...
      const schedules = requestedBatchId
        ? (db.schedules || []).filter((schedule) => schedule.batchId === requestedBatchId)
        : activeScheduleRows(db.schedules);
      if (requestedBatchId && !schedules.length) {
        return json(res, 404, { ok: false, reason: "排班版本不存在" });
      }
      return json(res, 200, {
        schedules,
        batches: summarizeScheduleBatches(db.schedules),
        totalStored: (db.schedules || []).length,
    ...
    if (req.method === "POST") {
      ...
      const db = await loadDb();
      const allSchedules = db.schedules || [];
      const activeSchedules = activeScheduleRows(allSchedules);
      const activeIds = new Set(activeSchedules.map((schedule) => schedule.id));
      const archivedSchedules = allSchedules.filter((schedule) => !activeIds.has(schedule.id));
      const currentBatch = currentScheduleBatch(allSchedules);
      const mode = body.mode || "new-batch";
      const now = new Date().toISOString();
      let savedSchedules;
      let replacedCenters = [];
      let newBatchId = currentBatch.id;

      if (mode === "replace-centers") {
        replacedCenters = [...new Set(result.schedules.map((schedule) => schedule.center))];
        const batchFields = currentBatch.id ? {
          batchId: currentBatch.id,
          batchName: currentBatch.name,
          batchCreatedAt: currentBatch.createdAt || now,
          isActive: true
        } : {};
        savedSchedules = [
          ...archivedSchedules,
          ...activeSchedules.filter((schedule) => !replacedCenters.includes(schedule.center)),
          ...result.schedules.map((schedule) => ({ ...schedule, ...batchFields }))
        ];
      } else if (mode === "new-batch") {
        newBatchId = crypto.randomUUID();
        const batchName = cleanBatchName(body.batchName, now);
        const previousBatchId = currentBatch.id || crypto.randomUUID();
        const previousBatchName = currentBatch.id
          ? currentBatch.name
          : cleanBatchName(body.previousBatchName || "上一版固定排班", currentBatch.createdAt || now);
        const archivedCurrent = activeSchedules.map((schedule) => ({
          ...schedule,
          batchId: schedule.batchId || previousBatchId,
          batchName: schedule.batchName || previousBatchName,
          batchCreatedAt: schedule.batchCreatedAt || schedule.importedAt || now,
          isActive: false,
          archivedAt: now
        }));
        savedSchedules = [
        ...
        return json(res, 400, { ok: false, reason: "固定排班导入模式不合法" });
      }

      await saveSchedules(savedSchedules);
      const active = activeScheduleRows(savedSchedules);
      return json(res, 200, {
        ok: true,
        count: active.length,
        totalStored: savedSchedules.length,
        importedCount: result.schedules.length,
        replacedCenters,
        batchId: newBatchId,
        schedules: active,
        batches: summarizeScheduleBatches(savedSchedules),
