...
async function handleApi(req, res) {
  ...
  if (route(pathname, "/api/checkins")) {
    ...
    if (req.method === "GET") {
      ...
      const unmatchedById = new Map(computeAttendance(db).unmatched.map((record) => [record.id, record.reason]));
      const checkins = (db.checkins || []).map((record) => ({
        ...record,
        matchStatus: unmatchedById.has(record.id) ? "异常" : "已匹配",
        matchReason: unmatchedById.get(record.id) || "已匹配固定排班"
      }));
