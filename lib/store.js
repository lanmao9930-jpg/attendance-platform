...
async function loadDb() {
  const planning = await planningStore.loadPlanningState();
  if (useCloudBaseStore()) {
    return {
      version: 3,
      planning,
      calendar: planning.calendar,
      schedules: planning.scheduleSnapshotId ? (await planningStore.readSnapshot(planning.scheduleSnapshotId)).schedules
        : await loadCloudBaseCollection(CLOUDBASE_COLLECTIONS.schedules),
  ...
  return { ...await loadLocalDb(), planning, calendar: planning.calendar };
}
...
async function loadScheduleHistory() {
  const state = await planningStore.loadPlanningState();
  const archives = state.batchArchives || [];
  if (useCloudBaseStore()) {
    const records = await loadCloudBaseCollection(CLOUDBASE_COLLECTIONS.scheduleHistory, "", "asc", { type: "batch" });
    return [...records.map(historySummary), ...archives];
  }
  if (useBlobStore()) {
  ...
  const db = await loadLocalDb();
  return [...(db.scheduleHistory || []).map(historySummary), ...archives];
}
...
async function loadScheduleHistoryBatch(batchId) {
  ...
  if (!id) return [];
  const state = await planningStore.loadPlanningState();
  const archive = (state.batchArchives || []).find(item => item.id === id);
  if (archive) return (await planningStore.loadImportSnapshots(archive.snapshotId)).schedules.map(row => ({ ...row, isActive: false }));
  if (useCloudBaseStore()) {
...
const planningStore = require("./cloud-planning").createPlanningStore({ cloudBase, useCloudBaseStore, loadLocalDb, saveLocalDb });

module.exports = {
  loadPlanningState: planningStore.loadPlanningState,
  commitPlanning: planningStore.commitPlanning,
  loadImportSnapshots: planningStore.loadImportSnapshots,
  addCheckin,
  loadDb,
  loadPhoto,
  loadScheduleHistory,
