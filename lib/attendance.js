...
function assignSwapRecords(checkins, occurrences, checkinIndex) {
  ...
  groupsByPerson.forEach((actualGroups, personKey) => {
    ...
    [...actualGroups.values()]
      ...
      .forEach((records) => {
        const exactCandidates = candidates.filter((schedule) => records.some((record) => (
        ...
        const describedCandidates = candidates.filter((schedule) => (
          declaration.includes(schedule.weekday) && declaration.includes(schedule.shift)
        ));
        const unusedCandidates = candidates.filter((schedule) => !usedScheduleIds.has(schedule.id));
        const candidatesWithoutNormalRecord = unusedCandidates.filter((schedule) => {
        ...
        const target = exactCandidates[0]
          || (describedCandidates.length === 1 ? describedCandidates[0] : null)
          || candidatesWithoutNormalRecord[0]
          || unusedCandidates[0];
        if (!target) return;
...
function computeAttendance(db, options = {}) {
  ...
  const swapAssignments = assignSwapRecords(checkins, occurrences, checkinIndex);
  const rows = occurrences.map((schedule) => {
    const relatedById = new Map([
      ...(checkinIndex.get(attendanceKey(schedule, schedule.shift)) || []),
  ...
  const unmatched = checkins.filter((record) => !matchedRecordIds.has(record.id)).map((record) => {
    ...
    const mismatchReason = explainUnmatched(record, occurrences);
    return {
      ...record,
      ...
      needsReview: true,
      reason: reportedSwap
        ? `学生申报调班，但未能关联本人固定排班：${record.swapTime || record.remark || mismatchReason}`
        : mismatchReason
    };
  });
